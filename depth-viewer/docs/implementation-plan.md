...
- `order_book_shared`テーブルに約6,000件以上のデータが存在
  - 2025年8月時点で約6,109件（約22日分）のデータ
  - データは5分間隔で記録（1日あたり288件）
...
#### 目標
Supabaseから全データを取得し、画面上にテーブル形式で表示する。

#### タスク
- **データ取得の実装**
  - Supabaseから全データを取得（制限なし or 10000件程度）
  - **重要**: `.limit()`を大きな値に設定し、全期間のデータを活用
...
- グリッド線とダークテーマの適用
  - 各グラフで独立したグリッド表示
  - X軸のグリッド線は両グラフで揃える（同じ時刻で線が引かれる）
  - Y軸のグリッド線は各グラフの値に応じて独立して表示
...
### 第4段階：タイムフレーム切り替え（時間足） 🎯難易度: 55/100

#### 目標
トレーディングチャートの「足」の概念を実装し、データの時間軸の粒度を切り替えられる機能を追加する。
**デスクトップアプリと完全に同じ仕様で実装する。**

#### タスク
- タイムフレーム選択ボタンの追加
- 5分足、15分足、1時間足、4時間足、1日足の選択肢
- **時刻ベースのデータグルーピング実装**
  - 全データ（6000件以上）に対してグループ化を実行
  - 各時間足に応じてデータをグループ化
  - 各グループから最後のデータ（終値）を採用
- **グラフ表示の制御**
  - グルーピング後のデータから最新300ポイントを表示
  - データ数が300未満の場合はすべて表示
- グラフの再描画処理
- 選択状態の保持（localStorage使用）
- UI配置の最適化（諸々の情報 → タイムフレーム → グラフ の順序）

#### データグルーピング仕様（デスクトップアプリ準拠）

**重要**: Supabaseには5分間隔のデータのみが存在するため、これをベースに各時間足を生成する。

##### グルーピングルール
- **5分足**: そのまま表示（グルーピングなし）
- **15分足**: 00分、15分、30分、45分にグループ化
- **1時間足**: 各時間の00分にグループ化（00:00、01:00、02:00...）
- **4時間足**: 00:00、04:00、08:00、12:00、16:00、20:00にグループ化
- **1日足**: 00:00にグループ化

##### データ採用ルール
各グループから**最後のデータ（終値/Close）**を採用する：
- 例1: 1時間足で13:00～13:55のデータがある場合 → 13:55のデータを13:00のポイントとして表示
- 例2: 4時間足で08:00～11:55のデータがある場合 → 11:55のデータを08:00のポイントとして表示
- 例3: 1日足で00:00～23:55のデータがある場合 → 23:55のデータをその日のポイントとして表示
...
6. 「4時間足」を選択：
   - 4時間ごとのデータポイント（6個/日）
   - 00、04、08、12、16、20時のデータ
7. 「1日足」を選択：
   - 1日ごとのデータポイント（1個/日）
   - 00:00のデータ
8. 各時間足で適切なグルーピングが行われている
9. ページリロード後も選択が保持される
...
- **重要**: タイムスタンプ表示時はタイムゾーン変換を行わない
  - `toLocaleString`使用時は`timeZone`オプションを指定しない
  - ISO形式のタイムスタンプから直接文字列として抽出
  - タイムゾーン情報（+00:00、Z）は除去する
- group_idは「default-group」でフィルタリング
- データは5分間隔で記録されている（1分足は存在しない）

### 3. タイムフレーム（時間足）の設計
- **実装する時間足**: 5分足、15分足、1時間足、4時間足、1日足
- **デフォルト値**: 1時間足（中期的なトレンドを把握しやすい）
- **選択状態の保持**: LocalStorageを使用してユーザーの選択を記憶
- **データグルーピング方式**: 
  - 時刻ベースでグループ化（例：1時間足なら00分にグループ化）
  - 各グループから最後のデータ（終値）を採用
  - デスクトップアプリ（coinglass_scraper.py）と同一ロジック
- **グラフ表示仕様**:
  - 横軸は最新300データポイント分を表示
  - データが300未満の場合はすべて表示
  - スクロールやズーム機能は将来的な拡張として検討

### 4. パフォーマンス
- **データ取得**: Supabaseから全データ（6000件以上）を取得
- **グラフ表示**: 最新300データポイントに制限
- **テーブル表示**: 最新100件に制限
- **メモリ管理**: 不要な古いデータは自動的にメモリから削除
- **再描画最適化**: グラフの再描画は最小限に抑える
